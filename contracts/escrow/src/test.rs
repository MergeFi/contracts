...
fn test_contribute_rejects_after_already_refunded() {
    ...
    client.fund(&204u64, &alice, &token_addr, &5_000i128, &1_000u64);

    // Admin-triggered refund before deadline (mock_all_auths) -> Refunded.
    env.ledger().set_timestamp(100);
    client.refund(&204u64);
    assert_eq!(client.get_escrow(&204u64).status, EscrowStatus::Refunded);

    let err = client.try_contribute(&204u64, &bob, &1_000i128);
